...
class LAMR_Tester(object):
    ...
    def __init__(self, log_dir, test_images):
        ...
        self.results_path = os.path.join("/data","mock_detections",os.path.basename(self.log_dir),"day","val")
    ...
    def __load_best_weights(self):
        best_weight = os.path.join(self.log_dir,"checkpoints", "best.pt")
        chkpt = torch.load(best_weight, map_location=self.device)
        self.yolov4.load_state_dict(chkpt)
        del chkpt

    def __load_weights(self,weight_path):
    ...
    def test(self):
        ...
        for i,(img_path, img, info_img) in tqdm(enumerate(self.dataloader),desc="Test to ECP... ", unit="imgs", total=len(self.dataloader)):
            ...
            os.makedirs(os.path.join(self.results_path,city_name),exist_ok=True)
            result_json_path = os.path.join(self.results_path,os.path.basename(img_path[0]).replace("png","json"))
...
if __name__ == "__main__":
    ...
    with open(args.split_file,"r") as f:
        ...
        test_images_list = split_fh["val"]
    LAMR_Tester(log_dir,test_images_list).test_series()
